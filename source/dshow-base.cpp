...
static HRESULT GetPinCategory(IPin* pin, GUID& category) {
    ...
    return propertySet->Get(AMPROPSETID_Pin, AMPROPERTY_PIN_CATEGORY,
                            nullptr, 0, &category, sizeof(GUID), &size);
}
...
static HRESULT DevicePathToDeviceInstancePath(const wchar_t* devicePath,
    ...
    transform(parseDevicePath.begin(), parseDevicePath.end(),
    ...
    /* Replace '#' by '\' */
    replace(parseDevicePath.begin(), parseDevicePath.end(), L'#',
            L'\\');
...
static HRESULT GetAudioCaptureParentDeviceInstancePath(IMoniker* audioCapture,
    ...
    HRESULT hr = audioCapture->BindToStorage(nullptr, nullptr, IID_IPropertyBag,
                                             (void**)&propertyBag);
    if (SUCCEEDED(hr)) {
...
static bool GetDeviceAudioFilterInternal(REFCLSID deviceClass,
    ...
    if (SUCCEEDED(hr)) {
        ...
        while (enumMoniker->Next(1, &moniker, &fetched) == S_OK) {
            ...
            if (samePath) {
                hr = moniker->BindToObject(nullptr, nullptr,
                                           IID_IBaseFilter,
