...
bool HDevice::SetupExceptionVideoCapture(IBaseFilter* filter,
    ...
    if (GetPinByName(filter, PINDIR_OUTPUT, L"TS Out", &pin))
...
bool HDevice::SetupVideoCapture(IBaseFilter* filter, VideoConfig& config) {
    ...
    if (config.name.find(L"C875") != string::npos ||
        config.name.find(L"Prif Streambox") != string::npos ||
        config.name.find(L"C835") != string::npos)
        return SetupEncodedVideoCapture(filter, config, AV_LGP);
    if (config.name.find(L"IT9910") != string::npos)
        return SetupEncodedVideoCapture(filter, config, HD_PVR_Rocket);
    if (config.name.find(HD_PVR1_NAME) != string::npos)
    ...
    if (videoConfig.format == VideoFormat::RGB32)
...
bool HDevice::FindCrossbar(IBaseFilter* filter, IBaseFilter** crossbar) {
    ...
    hr = builder->FindInterface(nullptr, nullptr, filter, IID_IAMCrossbar,
                                (void**)crossbar);
    if (SUCCEEDED(hr))
        return true;
...
bool HDevice::RenderFilters(const GUID& category, const GUID& type,
    ...
    hr = builder->RenderStream(&category, &type, filter, nullptr, capture);
    if (FAILED(hr)) {
