...
static bool Get_FORMAT_WaveFormatEx_Data(AudioInfo& info,
    ...
    case 16: info.format = AudioFormat::Wave16bit;
        break;
    case 32: info.format = AudioFormat::WaveFloat;
        break;
    }
...
struct ClosestVideoData {
    ...
    ClosestVideoData(VideoConfig& config, MediaType& mt)
        : config(config)
        , mt(mt)
        , bestVal(0)
        , found(false) {
...
static inline int GetFormatRating(VideoFormat format) {
    if (format >= VideoFormat::I420 && format < VideoFormat::YVYU)
        return 0;
    if (format >= VideoFormat::YVYU && format < VideoFormat::MJPEG)
        return 5;
...
struct ClosestAudioData {
    ...
    ClosestAudioData(AudioConfig& config, MediaType& mt)
        : config(config)
        , mt(mt)
        , bestVal(0)
        , found(false) {
...
static bool EnumDevice(const GUID& type, IMoniker* deviceInfo,
    ...
    hr = deviceInfo->BindToStorage(nullptr, nullptr, IID_IPropertyBag,
                                   (void**)&propertyData);
    if (FAILED(hr))
        return true;
    ...
    propertyData->Read(L"DevicePath", &devicePath, nullptr);

    hr = deviceInfo->BindToObject(nullptr, nullptr, IID_IBaseFilter,
                                  (void**)&filter);
    if (SUCCEEDED(hr)) {
